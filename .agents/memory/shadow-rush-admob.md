...
## Building/fetching the APK (no local Android SDK in this workspace)
- There is no Android SDK/gradle locally — APK must be built via GitHub Actions (`.github/workflows/build-android-apk.yml` and `android-build.yml`, both trigger on push to `main` touching shadow-rush paths, or `workflow_dispatch`).
- The repo's git remote is named `github` (not `origin`), so the `gitPush` callback's auto-detect fails with `NO_REMOTE`. Push via plain `git push github main` in ShellExec instead.
- To poll run status / download the artifact zip programmatically, read the token straight out of `git config --get remote.github.url` inside an impure CodeExecution block (never log it) and call the GitHub REST Actions API (`/actions/runs`, `/actions/artifacts/{id}/zip`) with it — `GITHUB_PAT` env var in the sandbox is a different/invalid credential for this repo (returns 401 Bad Credentials).

## "Ad unit doesn't match format" on rewarded ads
- capacitor-community/admob v8 exposes **two separate, similarly-named** reward APIs: `reward` (prepareRewardVideoAd/showRewardVideoAd, events `onRewardedVideoAd*`) for standard "Rewarded" ad units, and `reward-interstitial` (prepareRewardInterstitialAd/showRewardInterstitialAd, events `onRewardedInterstitialAd*`) for "Rewarded interstitial" ad units — a distinct AdMob ad format.
- If code correctly calls the `reward` API but AdMob still throws "doesn't match format", the ad unit ID in the config was very likely created as the wrong type in the AdMob console (e.g. "Rewarded interstitial" instead of plain "Rewarded", or an Interstitial/Banner ID pasted into the wrong slot). Verify the ad unit's format column in AdMob → Anuncios; this is an account-config issue, not something fixable purely in app code.

## Real ad units returning no ads while test IDs work
- If test IDs (IS_TESTING=true) serve ads fine but real IDs (IS_TESTING=false) show LoadAdError / no fill, and the app-side wiring (App ID, ad unit IDs, AndroidManifest meta-data, AD_ID permission) all match — this is very likely an AdMob **account/app-approval-side** issue, not a code bug: newly created AdMob apps/ad units commonly need 24-48h (sometimes longer) before they start serving real ads, and Google may require the app to be linked to a live Play Store listing. Point the user to check their AdMob console (Apps → app status, and the ad units' "ready" state) rather than re-auditing the integration code.
